...
function addCollections(db){
	...
	var dbase = db.db("ngCateringService");
	dbase.createCollection("platillos");
	dbase.createCollection("clientes",function(err, res) {
	    if (err) throw err;
		console.log("Collection clientes created!");
		res.createIndex( { "cliente": 1 }, { unique: true } )
